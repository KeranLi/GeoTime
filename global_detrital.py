...
@Logs@20231009: analyze global detrital data
"""
...
df = pd.read_excel('./data/Puetz_1200K.xlsx')

age_206pb_divide_238pb = df['Published 206Pb/238U age (Ma)']
age_206pb_divide_238pb.dropna(inplace=True)

freq_age_206pb_divide_238pb = age_206pb_divide_238pb.value_counts()
freq_df = pd.DataFrame(freq_age_206pb_divide_238pb)
freq_df = freq_df.reset_index()
freq_df.columns = ['age', 'frequency']
freq_df = freq_df.sort_values(by='age', inplace=False)

age = freq_df['age']
frequency = freq_df['frequency']

# check the shapes of Frequency and Age by comparing their colunn lenth
# If their lenth are same, print
if len(age) == len(frequency):
    print('----------The Age shape is: {}'.format(age.shape), end='----------\n')
    print('----------The Frequency shape is: {}'.format(frequency.shape), end='----------\n')
    print('----------Congratulations! Same shapes cause a successful loading! The length of Age and Frequency are same-----------')
else:
    print('----------The Age shape is: {}'.format(age.shape), end='----------\n')
    print('----------The Frequency shape is: {}'.format(frequency.shape), end='----------\n')
    print('----------Keep trying! Different shapes cause a failed loading! The length of Age and Frquency are not same----------')

# plot the bar histgram of the Age and Frequency by bar_plot.py
bar(
    np.array(age),
    np.array(frequency),
    title='The global detrital statistics of Frequency vs. Age',
    auto_axis=False,
    xlimit=None,
    ylimit=None
)
...
car_40_per_wa_trend = wa.rolling(frequency, window=5, min_periods=1, center=True)
